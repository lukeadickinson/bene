...
class TCP(Connection):
    ...
    def __init__(self, transport, source_address, source_port,
                 destination_address, destination_port, fastRetransmit=True, app=None, window=1000,drop=[]):
    ...
    def cwnd_sequence(self,windowSize,event):
        if self.node.hostname =='n1':
            Sim.plot('cwnd.csv','%s,%s,\n' % (Sim.scheduler.current_time(),windowSize))
    ...
    def handle_ack(self, packet):
            ...
            if self.lastAckReceived != packet.ack_number:
               ...
               self.duplicateAckCount = 0
                ...
                if self.duplicateAckCount == 3:
                    ...
                    #make sure we are multiple of mss
                    threshold_string = str(self.threshold)
                    self.threshold -= int(threshold_string[-3:])

                    self.window = self.mss
                    self.cwnd_sequence(self.window,'slow start')
        ...
        if self.sequence < packet.ack_number :
            ...
            if(self.window < self.threshold) :
                #slow start
                self.increment += min(self.mss, newBytes)
                while(self.increment >=self.mss):
                    self.window += self.mss
                    self.increment -= self.mss
                self.cwnd_sequence(self.window,'slow start step')

            else:
                #Additive Increase
                self.increment += self.mss * newBytes/self.window
                while(self.increment >=self.mss):
                    self.window += self.mss
                    self.increment -= self.mss
                self.cwnd_sequence(self.window,'Additive Increase')
